...
export default function Timer({ isCorrect, remainSeconds, setRemainSeconds, setTimeup, setLose }) {
  ...
  useEffect(() => {
    setTimeLeft()
    if (isCorrect || localStorage.getItem("win") || localStorage.getItem("score") == 0) {
      pause()
      time = new Date()
  ...
  return (
    <div className="flex w-full justify-center items-center h-48 space-x-4">
